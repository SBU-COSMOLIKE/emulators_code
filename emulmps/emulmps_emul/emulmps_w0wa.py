...
class PkEmulator:
    ...
    def get_pks(self, params: List[float], use_syren: bool = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        The returned k and P(k) are in units compatible with Cobaya's PowerSpectrumInterpolator,
        which expects the same units as the CAMB training data.
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray]: (k_modes, z_modes, P(k,z)).
                k_modes and P(k,z) are in units consistent with CAMB/PowerSpectrumInterpolator.
        """
        # Extract h for unit conversions
        params_array = np.array(params, dtype=np.float32)
        h = params_array[2] / 100.0  # H0 is 3rd parameter (index 2)
        ...
        if use_syren is True:
            ...
            # UNIT CONVERSION FOR SYREN:
            # Syren (plin_emulated) uses different conventions than CAMB training data.
            # Need to convert k and P(k) to match what PowerSpectrumInterpolator expects.
            # 
            # Empirically determined conversions (from unit analysis):
            # - k needs: k_out = k_syren * h
            # - P(k) needs: P_out = P_syren / h³
            #
            # This accounts for differences in how Syren and CAMB interpret the k array
            # and return P(k) values.
            # TO DO: Retrain NN using syren with corrected units and update to remove this
            # ad hoc correction
            k_out = self.K_MODES * h
            pks_out /= h**3
            ...
            # NN emulator was trained on CAMB data, so no conversion needed
            k_out = self.K_MODES
            pks_out = pks

        # Return the k-modes, z-modes, and the P(k,z) array
        return k_out, self.Z_MODES, pks_out
