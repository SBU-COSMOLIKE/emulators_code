...
#VM ENDS
    ...
    # Get linear P(k)
    if Plin_in is None:
        plin = linear.plin_emulated(k, sigma8, Om, Ob, h, ns, a=a, 
                                    emulator=emulator, 
...
_THETA_NEFF = np.array([
    1.65139294e+00, 4.88150280e+00, 5.12499000e-01, 1.48848000e-01,
    1.56499400e+01, 2.39307000e-01, 1.34631000e-01
], dtype=np.float64)

_B_C = np.array([
    0.335853, 1.42946178682748, 0.115256188211481, 0.057211, 48.072159, 0.194058,
    1.176006, 1.015136, 0.235398, 0.359587, 2.389843, 0.356875, 0.443138
], dtype=np.float64)
...
def ksigma_emulated_vec(sigma8, Om, Ob, h, ns, a):
    ...
    return np.exp(ksigma)

def neff_emulated_vec(sigma8, Om, Ob, h, ns, a):
    a = np.asarray(a, dtype=np.float64)
    t = _THETA_NEFF
    return (t[0] * ns - t[1]) * (
        t[2] * Ob - t[3] * h + (t[4] * a) ** (-t[5] * Om - t[6] * sigma8)
    )
...
def run_halofit_vec(
    ...
    if a.ndim != 1:
        raise ValueError("a_array must be 1D (nz,)")
    ...
    neff   = neff_emulated_vec(sigma8, Om, Ob, h, ns, a)
    Cc     = C_emulated_vec(sigma8, Om, Ob, h, ns, a)

    # broadcast k vs a (nz, nk)
    y = k[None, :] / ksigma[:, None]
    ...
    an = (pars[0] + pars[1] * neff + pars[2] * ne2 + pars[3] * ne3 + pars[4] * ne4 - pars[5] * Cc)
    an = 10.0 ** an
    bn = 10.0 ** (pars[6] + pars[7] * neff + pars[8] * ne2 + pars[9] * Cc)
    cn = 10.0 ** (pars[10] + pars[11] * neff + pars[12] * ne2 + pars[13] * Cc)
    gamma = pars[14] + pars[15] * neff + pars[16] * Cc
    nu = 10.0 ** (pars[17] + pars[18] * neff)

    a3 = a * a * a
    Om_over_a3 = Om / a3
    Omz = Om_over_a3 / (Om_over_a3 + 1.0 - Om)
    ...
    # 2-halo params (nz,)
    alpha = np.abs(pars[22] + pars[23] * neff + pars[24] * ne2 + pars[25] * Cc)
    beta  = (pars[26] + pars[27] * neff + pars[28] * ne2 + pars[29] * ne3 + pars[30] * ne4 + pars[31] * Cc)
    ...
    # 1-halo prediction (nz, nk)
    deltaH2 = (an[:, None] * y ** (3.0 * f1[:, None])) / (
        1.0
        + bn[:, None] * y ** (f2[:, None])
        + (cn[:, None] * f3[:, None] * y) ** (3.0 - gamma[:, None])
    )
    deltaH2 /= (1.0 + nu[:, None] / (y * y))
    ph = deltaH2 * _TWO_PI2 * inv_k3[None, :]

    # 2-halo prediction (nz, nk)
    deltaL2 = (k3[None, :] * plin) / _TWO_PI2
