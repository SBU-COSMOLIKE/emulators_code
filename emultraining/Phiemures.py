...
PATH = "./trainedemu5000resnetdl/chisquareaxphipre1000k"#saving location of the model
n_epoch = 700
intdim = 6 #internal dimension of ResMLP *128
Nlayer = 6 # number of layer of ResMLP

transform_matrix=np.load('axtestnew/PCAaxprephi.npy',allow_pickle=True)#PCA transformation matrix
transform_matrix = torch.Tensor(transform_matrix).to(device)
extrainfo=np.load("axtestnew/extrainfoaxprephi.npy",allow_pickle=True)
#load in data
train_samples=np.load('axtestnew/train_uni_ax_ext_phi_input.npy',allow_pickle=True)#.astype('float32')# This is actually a latin hypercube sampling of 1mil points

input_size=len(train_samples[0])

validation_samples=np.load('axtestnew/vali_uni_ax_final.npy',allow_pickle=True)

train_data_vectors=np.load('/home/grads/yijie/train_uni_ax_ext_phi.npy',allow_pickle=True,mmap_mode='r+')

validation_data_vectors=np.log(np.load('axtestnew/vali_uni_ax_final_phi.npy',allow_pickle=True))
