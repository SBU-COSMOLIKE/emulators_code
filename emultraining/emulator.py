...
class ResCNN(nn.Module):
    def __init__(self, input_dim, output_dim, int_dim, cnn_dim, kernel_size):
        ...
        #self.Res4 = ResBlock(int_dim, int_dim)
        #self.Res5 = ResBlock(int_dim, int_dim)
        ...
        self.Act1 = activation_fcn(int_dim)#nn.Tanh()
        self.Act2 = activation_fcn(int_dim)#nn.Tanh()
        self.Act3 = activation_fcn(int_dim)#nn.Tanh()
        #self.Act5 = activation_fcn(int_dim)#nn.Tanh()
        #self.Act6 = activation_fcn(int_dim)#nn.Tanh()


        self.conv = nn.Conv1d(in_channels=1, out_channels=1, 
            kernel_size=kernel_size,stride=1,padding=(kernel_size-1)//2)
        # self.conv = nn.Conv1d(in_channels=1, out_channels=36, 
        #     kernel_size=80,stride=40,padding=52) # fixing kernel size and in dim, then setting stride=out_channels, we find that the number of channels must be odd, the kernel_size-out_channels>=2, and 2(padding) = 15-out_channels

        # sets tried:
        # size = 150, stride = 70, padding=32, channels=72
        # size = 150, stride = 60, padding=11, channels=72 # best so far!
        # size = 120, stride = 80, padding=52, channels=72
        # size = 150, stride = 80, padding=52, channels=36, test chi2=3.2
        # size = 60, stride = 50, padding = 57, channels=36
